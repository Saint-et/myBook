...
const Creative = () => {
    ...
    useEffect(() => {
        ...
        img.onload = function () {
            ...
            // console.log({ w: canvasContainerImg.width, h: canvasContainerImg.height });
            ...
            if (rect) {
                ...
                const x = positionTest.x * 2 + leftOffset || '50%'; // Position horizontale du texte (au centre du canevas)
                const y = positionTest.y * 2 + topOffset || '50%'; // Position verticale du texte (au centre du canevas)
    ...
    const handleMouseMoveResizing = (e) => {
        ...
        switch (isResizing) {
            ...
            case 'top-move':
                setPositionTest({
                    x: positionTest.x + deltaX,
                    y: positionTest.y + deltaY
                });
    ...
    //const [isImageLarger, setIsImageLarger] = useState({ height: 0, width: 0 });
    useEffect(() => {
        //const image = canvasContainerRef.current.getBoundingClientRect();
        if (zoom) {
            canvasContainerScrollRef.current.getBoundingClientRect();
        }
        //setIsImageLarger({ height: image.height, width: image.width });
        //if (image && container && zoom) {
        //    const isLarger = image.width + 300 >= container.width;
        //}
    }, [canvasContainerScrollRef, zoom]);

    //console.log(isImageLarger);
        ...
        <>
            ...
            <RemoveScroll style={{ width: '100%', height: '100vh' }}>
                ...
                <div className='rowDoublepositionCreative' style={{ width: '100%', display: 'flex', padding: 0, margin: 0, maxWidth: '100%', justifyContent: 'space-between' }}>
                    <div className='rowDoublepositionContentCreative' style={{ width: '20%', minWidth: 316, maxWidth: 316, height: '100vh', display: 'block' }} data-theme={localTheme} />
                ...
                <div className='rowDoublepositionContentCreative scrollbarCreative' style={{ width: '20%', overflow: 'auto', minWidth: 316, maxWidth: 316, justifyContent: 'start', alignItems: 'center', height: '90vh', position: 'fixed', top: 0, left: 0, zIndex: 2000, marginTop: 40, marginBottom: 40 }} data-theme={localTheme}>
                ...
                <div className='nav_bar_scndCreative' style={{ height: 40, bottom: 0, width: '100%', maxWidth: '100%', position: 'fixed', zIndex: 2000, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row' }} data-theme={localTheme}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <input min={0.5} max={10.0} step={0.1} value={zoom} onChange={(e) => { setZoom(e.target.value) }} style={{ minWidth: 200, marginLeft: 5 }} className="range-input" type="range" name="" id="" />
