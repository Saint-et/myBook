    ...
    <>
      {props.promise?.map((doc, index) => (
        <div className='long_card open-elementPage' key={doc.id}>
          ...
          <div className='long_cardHover' style={{ width: '100%', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }} onClick={() => { GetMyFileFromLocal(doc.id) }} data-theme={localTheme}>
            <div style={{ display: 'flex', alignItems: 'center' }} translate='no'>
              ...
              {doc.visibility == 1 && <FontAwesomeIcon style={{ color: 'green' }} icon={faEye} />}
